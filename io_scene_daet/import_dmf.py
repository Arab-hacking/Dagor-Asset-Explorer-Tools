...
OBJECT_ROT_LEGACY = Vector((radians(90), radians(0), radians(-90)))
...
def socket_by_name(sockets, *names):
	"""Return the first socket matching any of the given names.

	Blender 4.x renamed several shader sockets/nodes and also tightened how
	``node.inputs[name]`` resolves sockets.  This helper first tries Blender's
	collection lookup, then falls back to comparing both socket names and
	identifiers.
	"""
	for name in names:
	...
	raise KeyError(f"None of the sockets exist: {', '.join(names)}")


def set_node_input_default(node:bpy.types.Node, value, *names):
	socket = socket_by_name(node.inputs, *names)
	socket.default_value = value
	return socket


def link_to_node_input(node_tree:bpy.types.NodeTree, from_socket, node:bpy.types.Node, *input_names):
	to_socket = socket_by_name(node.inputs, *input_names)
	node_tree.links.new(from_socket, to_socket)
	return to_socket


def new_node_compat(nodes:bpy.types.Nodes, *node_type_names):
	last_error = None

	for node_type_name in node_type_names:
		try:
			return nodes.new(node_type_name)
...
def sanitize_normal(normal:tuple):
	try:
...
def set_custom_normals_safe(mesh:bpy.types.Mesh, vertex_normals:list):
	"""Apply imported normals without aborting the whole import on bad data.

	Some DMF files contain zero/invalid normals for small aircraft details.
	Blender 4.x is stricter about custom normal data, so sanitize normals and
	fallback to loop-normal assignment when needed. If all methods fail, import
	the geometry anyway and let Blender calculate normals itself.
	"""
	if vertex_normals is None or len(vertex_normals) == 0:
		return

	normals = [sanitize_normal(normal) for normal in vertex_normals]
	...
	try:
		...
		elif hasattr(mesh, "normals_split_custom_set"):
			loop_normals = [normals[loop.vertex_index] for loop in mesh.loops]
			mesh.normals_split_custom_set(loop_normals)
	except Exception as exc:
		print(f"[W] Could not apply custom normals for mesh '{mesh.name}': {exc}")
...
def set_skeleton_transform(skeleton:Skeleton, use_legacy_rotation:bool = False):
	set_object_scale(skeleton.armature_obj, use_legacy_rotation)
...
def create_normal_node(nodes, node_tree, principled_bsdf):
	...
	separate_rgb_node = new_node_compat(nodes, "ShaderNodeSeparateColor", "ShaderNodeSeparateRGB")
	link_to_node_input(node_tree, normal_node.outputs["Color"], separate_rgb_node, "Color", "Image")
	node_tree.links.new(socket_by_name(separate_rgb_node.outputs, "Blue", "B"), socket_by_name(principled_bsdf.inputs, "Metallic"))

	combine_rgb_node = new_node_compat(nodes, "ShaderNodeCombineColor", "ShaderNodeCombineRGB")
	node_tree.links.new(normal_node.outputs["Alpha"], socket_by_name(combine_rgb_node.inputs, "Red", "R"))
	node_tree.links.new(socket_by_name(separate_rgb_node.outputs, "Green", "G"), socket_by_name(combine_rgb_node.inputs, "Green", "G"))
	set_node_input_default(combine_rgb_node, 1.0, "Blue", "B")
	
	normalmap_node = nodes.new("ShaderNodeNormalMap")
	node_tree.links.new(socket_by_name(combine_rgb_node.outputs, "Color", "Image"), normalmap_node.inputs["Color"])
	node_tree.links.new(normalmap_node.outputs["Normal"], socket_by_name(principled_bsdf.inputs, "Normal"))

	invert_node = nodes.new("ShaderNodeInvert")
	node_tree.links.new(normal_node.outputs["Alpha"], invert_node.inputs["Color"])
	node_tree.links.new(invert_node.outputs["Color"], socket_by_name(principled_bsdf.inputs, "Specular IOR Level", "Specular"))
...
def create_materials(f:BufferedReader, 
	...
	for _ in range(material_count):
		...
		if shader_class == "rendinst_tree_colored":
			...
			detail_tex = DetailTexture(1, detail_diffuse, detail_normal)
		...
		set_node_input_default(principled_bsdf, diff, "Base Color")
		# principled_bsdf.inputs["Emission"].default_value = emis # some textures appear completely white with this
		set_node_input_default(principled_bsdf, 1.0, "Roughness")
		...
		link_to_node_input(node_tree, diffuse_node.outputs["Color"], principled_bsdf, "Base Color")
		...
		if hasMask:
			invert_node = nodes.new("ShaderNodeInvert")
			node_tree.links.new(socket_by_name(separate_rgb_node.outputs, "Blue", "B"), invert_node.inputs["Color"])
		...
		if hasMask:
			...
			link_to_node_input(node_tree, alpha_output, principled_bsdf, "Alpha")
		...
		for detail_tex in detail:
			...
			if detail_tex.normal is not None:
				...
				set_node_input_default(detail_bsdf, diff, "Base Color")
				set_node_input_default(detail_bsdf, emis, "Emission Color", "Emission")
				set_node_input_default(detail_bsdf, 1.0, "Roughness")
				...
				link_to_node_input(node_tree, detail_diffuse_node.outputs["Color"], detail_bsdf, "Base Color")
...
def create_mesh(f:BufferedReader, name:str, global_verts:tuple, global_uvs:tuple, global_normals:tuple):
	...
	# Blender 3.6+ added a fourth ``shade_flat`` argument.  Use it where
	# available, but keep the fallback so the code is still importable in older
	# Blender versions.
	...
	set_custom_normals_safe(mesh, obj_normals)
...
def set_object_scale(ob:bpy.types.Object, use_legacy_rotation:bool = False):
	ob.scale = OBJECT_SCALE
	ob.rotation_euler = OBJECT_ROT_LEGACY if use_legacy_rotation else OBJECT_ROT
...
def set_object_transform(ob:bpy.types.Object,
			skeleton:Skeleton,
			apply_scale:bool,
			skinned:bool,
			relative_parenting:bool,
			apply_bone_transform_to_skinned:bool = False,
			use_legacy_rotation:bool = False):
	obj_name = ob.name
	...
	if skeleton is not None:
		...
		if skeleton.hasBone(obj_name):
			if not skinned or apply_bone_transform_to_skinned:
			...
			if apply_scale:
				set_object_scale(ob, use_legacy_rotation)
				apply_object_transform(ob)
...
def load(filepath:str, 
	...
	with open(filepath, "rb") as file:
		...
		if skeleton is not None and apply_scale:
			set_skeleton_transform(skeleton, use_legacy_rotation)
		...
		for i in range(obj_count):
			...
			set_object_transform(ob,
				skeleton,
				apply_scale,
				skinned,
				relative_parenting,
				apply_bone_transform_to_skinned,
				use_legacy_rotation)
