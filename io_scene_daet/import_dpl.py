		...
		for k in range(ent_cnt):
			...
			try:
				load_dmf(ent_path,
					...
					apply_bone_transform_to_skinned = apply_bone_transform_to_skinned,
			...
			processed_cnt = min(pos_matrix_cnt, max_occurences_cnt)
			
			for i in range(processed_cnt):
			...
			remaining_cnt = pos_matrix_cnt - processed_cnt
			
			if remaining_cnt > 0:
				print(f"[D] \tSkipping {remaining_cnt} other occurences (max occurences reached)")
				skip_model(f, remaining_cnt) 
