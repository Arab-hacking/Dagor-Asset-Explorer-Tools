...
class ImportDMF(bpy.types.Operator, ImportHelper):
	...
	apply_scale: BoolProperty(
		name = "Fix transform",
		description = "Scale model by (-40, 40, 40) and rotate it to Blender coordinates. Use Legacy -90° Z rotation below if needed",
		default = True,
	)
	...
	apply_bone_transform_to_skinned: BoolProperty(
		name = "Force skinned bone transforms",
		description = "Experimental: also apply node/bone transforms to meshes marked as skinned. Enable this if aircraft details or moving parts import at the origin or in wrong places",
		default = False,
	)

	use_legacy_rotation: BoolProperty(
		name = "Legacy -90° Z rotation",
		description = "Use the older transform preset (90, 0, -90) instead of the current preset (90, 0, 180). Try this if the model hierarchy is rotated or offset incorrectly",
		default = False,
	)
...
class DMF_PT_import_transform(bpy.types.Panel):
	...
	def draw(self, context):
		...
		layout.prop(operator, "apply_bone_transform_to_skinned")
		layout.prop(operator, "use_legacy_rotation")
...
class ImportDPL(bpy.types.Operator, ImportHelper):
	...
	apply_bone_transform_to_skinned: BoolProperty(
		name = "Force skinned bone transforms",
		description = "Experimental: pass the same transform workaround to DMF files loaded by this DPL",
		default = False,
	)
